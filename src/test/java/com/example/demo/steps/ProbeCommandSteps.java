...
public class ProbeCommandSteps extends CommonSteps {
  ...
  public void createPayloadWithPlaceholders(String payloadKey, String valueKey, String rawPayload) throws Exception {
    ...
    enrichedPayload = Pattern.compile("\\$\\{" + valueKey + "\\}").matcher(enrichedPayload).replaceAll(keyValue.toString());
  ...
  public void verifyDatabaseContainsPositions(io.cucumber.datatable.DataTable dataTable) {
    ...
    data.forEach(
      row -> {
        ...
        assertTrue(
          probeVisitedPositionsRepo.existsByXCoordinateAndYCoordinateAndDirection(x, y, Direction.valueOf(direction.toUpperCase())));
      });
  }
